# I am Luan De La Vega



## Backend Developer | Java | Spring | Microservices | RESTful APIs | AWS | DevOps


------
Desenvolvedor Back-end com 4 anos de experiência em Java. Possui vasta experiência em frameworks Spring, como Spring Boot, Spring MVC, Spring Data e Spring Security. Atuou na criação, manutenção e melhorias em APIs, microsserviços, JDBC, JPA e Hibernate. Experiência com bancos de dados relacionais (PostgreSQL, MySQL) e NoSQL (MongoDB, Redis). Implementou melhorias de performance de código utilizando threads. Possui conhecimento em WebFlux, cacheamento com Redis e implementação de AWS (SQS AWS com filas FIFO). Atua em conformidade com padrões de design como SOLID e Clean Code. Experiência com testes de performance e stress com JMeter, testes unitários com JUnit 5 e Mockito. Atuação diária com CI/CD, Jenkins, SonarQube, Veracode, Git e Bitbucket.

Habilidades:

Java 8, 11, 17, 21 e 25.
Spring Boot, Spring MVC, Spring Security, Spring Data
APIs, microsserviços
JDBC, JPA, Hibernate
PostgreSQL, MySQL, MongoDB, Redis
Threads, WebFlux
Cacheamento com Redis
AWS SQS, RabbitMq, AWS S3, AWS ElastiCache
SOLID, Clean Code, FIRST
Testes de performance e stress com JMeter
Testes unitários com JUnit 5 e Mockito
CI/CD, Jenkins, SonarQube, Veracode
Git, GitLab
Angular

-----


### Languages and Tools:

<img align="left" alt="Visual Studio Code" width="36px" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/visual-studio-code/visual-studio-code.png" />
<img align="left" alt="Intellij" width="36px" src="https://cdn.icon-icons.com/icons2/3053/PNG/512/intellij_macos_bigsur_icon_190061.png" />

<img align="left" alt="Java" width="40px" src="https://cdn.icon-icons.com/icons2/2415/PNG/512/java_original_wordmark_logo_icon_146459.png" />
<img align="left" alt="Git" width="36px" src="https://cdn.icon-icons.com/icons2/2415/PNG/512/git_plain_wordmark_logo_icon_146508.png" />
<img align="left" alt="GitHub" width="36px" src="https://cdn.icon-icons.com/icons2/1181/PNG/512/1490128251-epic-social-media-online-web-internet-github_82122.png" />

<br />


------

</details>

